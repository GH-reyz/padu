...
cat > /usr/local/etc/xray/$user-VLESS-TLS.yaml <<EOF
port: 7890
socks-port: 7891
redir-port: 7892
mixed-port: 7893
...
cat > /usr/local/etc/xray/$user-VLESS-NTLS.yaml <<EOF
port: 7890
socks-port: 7891
redir-port: 7892
mixed-port: 7893
...
cat > /usr/local/etc/xray/$user-VLMAXIS.yaml <<EOF
port: 7890
socks-port: 7891
redir-port: 7892
mixed-port: 7893
...
proxy-groups:
  - name: GH-reyz-AUTOSCRIPT
    type: select
    proxies:
      - XRAY_VLESS_MAXIS_${user}
      - DIRECT
rules:
  - MATCH,GH-reyz-AUTOSCRIPT
EOF

cat > /usr/local/etc/xray/$user-VLDIGI.yaml <<EOF
port: 7890
socks-port: 7891
redir-port: 7892
mixed-port: 7893
...
# Move Yaml To html
mv /usr/local/etc/xray/$user-VLESS-TLS.yaml /home/vps/public_html/$user-VLESS-TLS.yaml
mv /usr/local/etc/xray/$user-VLESS-NTLS.yaml /home/vps/public_html/$user-VLESS-NTLS.yaml
mv /usr/local/etc/xray/$user-VLMAXIS.yaml /home/vps/public_html/$user-VLMAXIS.yaml
mv /usr/local/etc/xray/$user-VLDIGI.yaml /home/vps/public_html/$user-VLDIGI.yaml
clear
echo -e ""
