...
trojanlink3="trojan://${uuid}@104.17.113.188:${tr}?type=ws&security=tls&host=${domain}&path=wss://who.int/trojan-tls&sni=who.int#XRAY_TROJAN_MAXIS_${user}"
trojanlink4="trojan://${uuid}@162.159.134.61:${trnone}?type=ws&security=none&host=${domain}&path=/trojan-ntls#TROJAN_DIGI_${user}"

cat > /usr/local/etc/xray/$user-TROJAN-TLS.yaml <<EOF
port: 7890
socks-port: 7891
redir-port: 7892
mixed-port: 7893
...
proxies:
  - name: XRAY_TROJAN_TLS_${user}
    ...
    udp: true
proxy-groups:
  - name: GH-reyz-AUTOSCRIPT
    type: select
    proxies:
      - XRAY_TROJAN_TLS_${user}
      - DIRECT
rules:
  - MATCH,GH-reyz-AUTOSCRIPT
EOF

cat > /usr/local/etc/xray/$user-TRMAXIS.yaml <<EOF
port: 7890
socks-port: 7891
redir-port: 7892
mixed-port: 7893
...
proxy-groups:
  - name: GH-reyz-AUTOSCRIPT
    type: select
    proxies:
      - XRAY_TROJAN_MAXIS_${user}
      - DIRECT
rules:
  - MATCH,GH-reyz-AUTOSCRIPT
EOF

cat > /usr/local/etc/xray/$user-TRDIGI.yaml <<EOF
port: 7890
socks-port: 7891
redir-port: 7892
mixed-port: 7893
...
# Move Yaml To html
mv /usr/local/etc/xray/$user-TROJAN-TLS.yaml /home/vps/public_html/$user-TROJAN-TLS.yaml
mv /usr/local/etc/xray/$user-TROJAN-NTLS.yaml /home/vps/public_html/$user-TROJAN-NTLS.yaml
mv /usr/local/etc/xray/$user-TRMAXIS.yaml /home/vps/public_html/$user-TRMAXIS.yaml
mv /usr/local/etc/xray/$user-TRDIGI.yaml /home/vps/public_html/$user-TRDIGI.yaml
clear
echo -e ""
