...
cat > /usr/local/etc/xray/$user-VLESS-DIRECT.yaml <<EOF
port: 7890
socks-port: 7891
redir-port: 7892
mixed-port: 7893
...
dns:
  ...
  fake-ip-filter:
    ...
    - +.media.dssott.com
proxies:
  - name: XRAY_VLESS_DIRECT443_${user}
    server: ${sts}${domain}
    port: ${xtls}
    type: vless
    uuid: ${uuid}
...
dns:
  ...
  fake-ip-filter:
    ...
    - +.media.dssott.com
proxies:
  - name: XRAY_VLESS_SPLICE443_${user}
    server: ${sts}${domain}
    port: ${xtls}
    type: vless
    uuid: ${uuid}
...
# Move Yaml To html
mv /usr/local/etc/xray/$user-VLESS-DIRECT.yaml /home/vps/public_html/$user-VLESS-DIRECT.yaml

mv /usr/local/etc/xray/$user-VLESS-SPLICE.yaml /home/vps/public_html/$user-VLESS-SPLICE.yaml

mv /usr/local/etc/xray/$user-VLESS-DIRECT443.yaml /home/vps/public_html/$user-VLESS-DIRECT443.yaml

mv /usr/local/etc/xray/$user-VLESS-SPLICE443.yaml /home/vps/public_html/$user-VLESS-SPLICE443.yaml
clear
echo -e ""
